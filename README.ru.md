...
> **Текущий проверенный результат P2P:** Static BAR1 между GPU1 `82:00.0` и GPU2 `83:00.0` дал **5.30 GB/s в одну сторону**, **10.27 GB/s bidirectional** и **1.69–1.71 мкс** на PCIe Gen2 x16. Прошли CUDA peer copy, прямое SM чтение и запись в обе стороны. [Полный русский отчёт](docs/STATIC-BAR1-P2P.ru.md) · [English report](docs/STATIC-BAR1-P2P.md)
...
Этот этап нужен владельцам двух и более карт для прямого GPU↔GPU обмена, например для `llama.cpp` tensor split. Текущий подтверждённый вариант — [Static BAR1 P2P](docs/STATIC-BAR1-P2P.ru.md).

> **Исправление истории:** старые цифры Mailbox B2 6.69–6.70 GB/s оказались ложноположительными. Поздний content-check показал, что VRAM соседней карты не менялась, а NCCL мог зависать. Mailbox B2 сохранён только как исторический recovery-артефакт; не используйте его как рабочий P2P.
...
После восстановления потерянного системного диска Mailbox B2 был собран на
целостной Bayley-базе, сохраняющей Gen2, 64 ГБ BAR1 и FBPA unlock. Четыре
последовательных прогона дали:

```text
one-way:      6.69–6.70 GB/s
bidirectional: 13.37–13.40 GB/s
GPU latency:   1.54–1.62 us
```

Для сравнения Bayley static BAR1 на тех же двух картах Gen2 x16 дал 5.30 GB/s,
10.28 GB/s и 1.68–1.73 мкс. Полные исходники и аварийное восстановление:
