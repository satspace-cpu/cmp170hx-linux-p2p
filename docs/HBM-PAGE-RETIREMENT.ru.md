...
```text
physical = 0x12C00000 + (0xABA44677F - 0xF0000)
         = 0xACCF5677F
```
...
Ошибка находилась внутри неё на смещении `0x677F`. После исключения страницы
...
место дефекта. Поэтому адрес вроде `0xABA44677F` нельзя напрямую использовать
